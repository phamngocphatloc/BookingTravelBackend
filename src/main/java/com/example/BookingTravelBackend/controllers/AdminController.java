...
public class AdminController {
    ...
    @PutMapping("setRole")
    public ResponseEntity<HttpRespone> updateAdmin(@RequestParam("userId") int userId, @RequestParam("roleName") String roleName) {
        UserDetailsResponse respose = userService.updateAdmin(userId, roleName);
        return ResponseEntity.ok(new HttpRespone(HttpStatus.OK.value(), "success", respose));
    }

    @GetMapping("getRoomById")
    public ResponseEntity<HttpRespone> getRoom(@RequestParam int roomId) {
        RoomRespone response = roomService.selectRoomById(roomId);
        return ResponseEntity.ok(new HttpRespone(HttpStatus.OK.value(),
                "success", response));
    }

    @PostMapping ("updateRoom")
    public ResponseEntity<HttpRespone> updateRoom (@Valid @RequestBody RoomEditRequest request){
        RoomRespone response = roomService.updateRoom(request);
        return ResponseEntity.ok(new HttpRespone(HttpStatus.OK.value(),
                "success", response));
    }
